...
import { updatePlan, fetchPlanById } from "@/redux/slices/planSlice";
...
export default function EditPlan() {
  ...
  useEffect(() => {
    // Fetch plan data and categories when component mounts
    const loadData = async () => {
      try {
        ...
        // Fetch categories
        await dispatch(fetchCategories({ page: 1, limit: 100 })).unwrap();
  ...
  // Update form when currentPlan is loaded
  useEffect(() => {
    if (currentPlan && currentPlan.id === parseInt(id)) {
      setFormData({
        plan_name: currentPlan.plan_name || "",
        coin_package: currentPlan.coin_package?.toString() || "",
        base_price: currentPlan.base_price?.toString() || "",
        category_id: currentPlan.category_id?.toString() || "",
        discount_percentage: currentPlan.discount_percentage?.toString() || "",
  ...
  const handleSubmit = async (e) => {
    ...
    console.log("Updating plan data:", submitData);
          ...
          <div className="flex justify-between items-center p-6 border-b border-gray-200">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                Update plan details and pricing information
              </p>
            </div>
            <Button 
              type="button" 
